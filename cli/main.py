# -*- coding: utf-8 -*-
#文件位置：cli/main.py
#命令行入口：提供repl 子命令，用于加载DSL、接入真假LLM、循环对话
...
from dsl.parser import parse_program    #解析DSL
from runtime.executer import Interpreter    #执行器
...
console = Console() #rich控制台

@app.command()
def repl(
    script: str = typer.Option(..., help="DSL脚本文件路径，如samples/logistics.dsl")
):
    """
    交互REPL：
    读取DSL脚本，构建解释器
    使用LLM路由意图
    根据意图进入对应状态，匹配规则并给出回复
    用户输入'exit'退出
    """
    ...
    #创建LLM路由（内置自动fallback到Mock）
    router = OpenAICompatRouter()

    console.print("[green]DSL已加载，输入你的问题，输入'exit'退出。[/green]")
